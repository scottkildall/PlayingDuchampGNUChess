...
int BookBuilderOpen(void)
{
  ...
  if ((rfp = fopen(BOOKRUN,"rb")) != NULL) {
    dbg_printf("Opened existing book!\n");
    if (!check_magic(rfp)) {
      dbg_printf( "File %s does not conform to the current format.\n"
    ...
    if (res != BOOK_SUCCESS) {
      fclose(rfp);
      return res;
    dbg_printf("Read %d book positions\n", bookcnt);
    dbg_printf("Got %d hash collisions\n", bookhashcollisions);
  } else {
    wfp = fopen(BOOKRUN,"w+b");
    if (wfp == NULL) {
      dbg_printf( "Could not create %s file: %s\n",
		 BOOKRUN, strerror(errno));
      return BOOK_EIO;
    }
    if (write_magic(wfp) != BOOK_SUCCESS) {
      dbg_printf( "Could not write to %s: %s\n",
	      BOOKRUN, strerror(errno));
      return BOOK_EIO;
    }
    if (fclose(wfp) != 0) {
    ...
    dbg_printf("Created new book %s!\n", BOOKRUN);
    rfp = fopen(BOOKRUN, "rb");
    if (rfp == NULL) {
      dbg_printf( "Could not open %s for reading: %s\n",
	      BOOKRUN, strerror(errno));
      return BOOK_EIO;
    }
    digest_bits = MAX_DIGEST_BITS;
...
int BookQuery(int BKquery)
{
  ...
  if (!bookloaded) {
    ...
    if (rfp == NULL) {
      ...
      if (!(flags & XBOARD) || BKquery == 1)
        dbg_printf(" No book found.\n\n");
      return BOOK_ENOBOOK;
    }
    if (!(flags & XBOARD)) {
      dbg_printf( "Loading book from %s.\n", *booktry);
    }
    /*
    ...
    if (!(flags & XBOARD))
      dbg_printf("%d hash collisions... ", bookhashcollisions);
  }      
  ...
  for (i = 0; i < icnt; i++) {
	...
	if (mcnt >= MAXMATCH) {
	  dbg_printf(" Too many matches in book.\n\n");
	  goto fini;
	}
...
fini:  
  if (!(flags & XBOARD) || BKquery == 1)
  {
    dbg_printf(" Opening database: %d book positions.\n",bookcnt);
    if (mcnt+1 == 0) {
      dbg_printf(" In this position, there is no book move.\n\n");
    } else if (mcnt+1 == 1) {
      dbg_printf(" In this position, there is one book move:\n");
    } else {
      dbg_printf(" In this position, there are %d book moves:\n", mcnt+1);
    }
  }
  /* No book moves */
  if (mcnt == -1) {
  ...
  if (mcnt+1) {
	...
	if (!(flags & XBOARD) || BKquery == 1) {
	  SANMove(pref[i].move,1);
          dbg_printf(" %s(%d) ",SANmv,pref[i].score);
	}
