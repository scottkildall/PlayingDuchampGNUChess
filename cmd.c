...
void cmd_depth(void)
{
  ...
  dbg_printf("Search to a depth of %d\n",SearchDepth);
}
...
{
  if ( flags & XBOARD ) {
    dbg_printf("tellusererror command 'edit' not implemented\n");
    fflush(stdout);
  }
}
...
{
  if (token[1][0] == 0) {
    dbg_printf("Current HashSize is %d slots\n", HashSize);
  } else {
    int i;
    /* i = atoi (token[1]); */
    errno = 0;
    ...
    if ( errno != 0 || *endptr != '\0' ){
      dbg_printf("Hashsize out of Range or Invalid\n");
    }
    else {
     CalcHashSize(i);
     InitHashTable (); 
...
void cmd_hint(void)
{
  if ( flags & ENDED ){
    ...
    if (IsLegalMove(HintMove)){
      ...
      dbg_printf("No hint available at this time\n");
    }
  }
  fflush(stdout);
}
...
{
  if (token[1][0] == '?') {
    dbg_printf("name    - list known players alphabetically\n");
    dbg_printf("score   - list by GNU best result first \n");
    dbg_printf("reverse - list by GNU worst result first\n");
  } else {
...
void cmd_name(void)
{
  ...
  for (suffix = 0; suffix < 1000; suffix++) {
    ...
    if (access(logfile,F_OK) < 0) {
      ...
      if (ofp == NULL) {
	ofp = stdout;
	dbg_printf( "Failed to open %s for writing: %s\n", 
		logfile, strerror(errno));
      }
      return;
    }
  }
  dbg_printf( "Could not create logfile, all slots occupied.\n");
  dbg_printf( "You may consider deleting or renaming your existing logfiles.\n");
}
...
void cmd_protover(void)
{
  if (flags & XBOARD) {
    ...
    dbg_printf("feature setboard=1 analyze=1 ping=1 draw=0 sigint=0"
...
void cmd_rating(void)
{
  ...
  dbg_printf("my rating = %d, opponent rating = %d\n",myrating,opprating); 
 ...
 ************************************************************************/
{
   ...
   {
      dbg_printf("My rating = %d\n",myrating);
      dbg_printf("Opponent rating = %d\n",opprating);
   } 
      ...
      GenMoves (1);      
      ShowMoveList (1);
      dbg_printf ("No. of moves generated = %ld\n", GenCnt);
   }
   ...
   else if (tokeneq (token[1], "eval") || tokeneq (token[1], "score"))
   {
      ...
      wk = ScoreK (white);
      bk = ScoreK (black);
      dbg_printf ("White:  Mat:%4d/%4d  P:%d  N:%d  B:%d  R:%d  Q:%d  K:%d  Dev:%d  h:%d x:%d\n",
	board.pmaterial[white], board.material[white], wp, ScoreN(white), 
      ...
      dbg_printf ("Black:  Mat:%4d/%4d  P:%d  N:%d  B:%d  R:%d  Q:%d  K:%d  Dev:%d  h:%d x:%d\n",
 ...
 **************************************************************************/
{
   ...
   if (strlen(token[1])>0) {
	 ...
	 if  (strncmp(*p, token[1], strlen(token[1])) == 0) {
      ...
      dbg_printf("Help for command %s not found\n\n", token[1]);
   }
   dbg_printf("List of commands: (help COMMAND to get more help)\n");
   for (p=helpstr, count=0; *p; p++) {
      len = strcspn(*p, " ");
      if (len > 0) {
	 count += dbg_printf("%.*s  ", len, *p);
	 if (count > 60) {
	    count = 0;
	    puts("");
	 }
 ...
 *************************************************************************/
{
   ...
   if (ptr != NULL) {
     ...
     dbg_printf("%d. ",GameCnt/2+1);
     dbg_printf("%s",token[0]);
     if (ofp != stdout) {
       dbg_printf("%d. ",GameCnt/2+1);
       dbg_printf("%s",token[0]);
     }
