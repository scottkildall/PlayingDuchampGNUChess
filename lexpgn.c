...
#line 179 "lexpgn.l"
{
	...
	if (yyleng >= (sizeof(tagname)-1)) {
		dbg_printf("Error, tagname too long: %s", yytext);
		return 1;
	}
...
#line 190 "lexpgn.l"
{
	dbg_printf("Bad character as tag name: %s\n", yytext);
	return 1;
	}
	YY_BREAK
case 21:
...
#line 196 "lexpgn.l"
{
	dbg_printf("Bad character as tag separator: %s\n", yytext);
	return 1;
	}
	YY_BREAK
case 23:
...
{ /* tag data */
	...
	switch(ctag) {
		...
		case FEN_TAG:
			...
			if (yyleng > 82) {
				dbg_printf("Error: FEN too long: %s\n", yytext);
				return 1;
			}
...
{ /* Process a move */
	...
	if (data_dest != DEST_TRASH) {
		...
		if (yyleng > 7) {
			dbg_printf("Error: move too long: %s\n", yytext);
			return 1;
		}
		...
		if (addtobook[side]) {
			if (BookBuilder (result, side) == BOOK_EFULL) {
				dbg_printf("Book full - Failed to add move %s\n",
				       yytext);
				ShowBoard();
				return 1;
			}
...
#line 414 "lexpgn.l"
{
	 dbg_printf("Illegal character %c in input stream.\n", yytext[0]);
	 return 1;
	}
	YY_BREAK
case 51:
