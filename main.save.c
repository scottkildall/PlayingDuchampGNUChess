...
//This is just here for testing, it should probably be replaced by some kind of error once development is done.
    ...
    //move="e5";
    //color="black";
    sprintf( gamefile, "/web/turbulence/cgi-bin/duchamp/games/%s.pgn", "kildall3" );
    startUp = true;
    //new = true;
    //undo = true;
  }
  else {
    char input[MAXINPUT], data[MAXINPUT]; //Needed for getting an manipulating input data
    ...
    //get rid of any encoding in the string.
  ...
  if( startUp || undo )  {
    ...
    if( startUp ) {printf("\nsavedGame=yes&pgn="); encode( buffer, buffer+lSize ); }
    else {
      char *src;
