...
//leslie changed to not append.  This writes the whole game, so there is no reason to append.
   fp = fopen (file, "w");
   if (fp == NULL)
   {
      dbg_printf ("Cannot write to file %s\n", file);
      return;
   }
 ...
 ****************************************************************************/
{
   ...
   if (fp == NULL)
   {
     dbg_printf("Cannot open file %s: %s\n", 
	     file, strerror(errno));
     return;
   }
   yyin = fp;
   ...
   while(1) {
     ...
     ngames++;
     if (ngames % 10 == 0) dbg_printf("Games processed: %d\r",ngames);
     fflush(stdout);
   }
   ...
   dbg_printf("Time = %.0f seconds\n", et);
   dbg_printf("Games compiled: %d\n",ngames);
   dbg_printf("Games per second: %f\n",ngames/et);
   dbg_printf("Positions scanned: %d\n",newpos+existpos);
   dbg_printf("Positions per second: %f\n",(newpos+existpos)/et);
   dbg_printf("New & unique added: %d positions\n",newpos);
   dbg_printf("Duplicates not added: %d positions\n",existpos);
}
