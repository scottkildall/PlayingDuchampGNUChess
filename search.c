 ...
 **************************************************************************/
{
   int best, score, savealpha;
	// PLAYING DUCHAMP --> added secondBest -- for Blundering
	int secondBest;
	int secondBestRootPV;
   ...
   best = -INFINITY;
	secondBest = -INFINITY;	// PLAYING DUCHAMP added
   savealpha = alpha;
   nodetype = PV;
   pbest = NULL;

	// ADD BLUNDER CODE HERE —— is SECOND BEST a good way to indicate a Blunder?
	// look at RootPV as this seems to store the moves
   for (p = TreePtr[1]; p < TreePtr[2]; p++)
   {
	 ...
	 nodetype = CUT;
      ...
      if (score > best)
      {
		  secondBest = best;	// PLAYING DUCHAMP, added
		  if( secondBest == -INFINITY )
			  secondBestRootPV = p->move;
		  else
			   secondBestRootPV = RootPV;
 ...
 *****************************************************************************/
	...
	/*if( duchampBlundered )
	{
		dbg_printf("SearchRoot() duchamp has blundered, RootPV = %d, SB_RootPV = %d, bestScore = %d, SB_Score = %d\n", RootPV, secondBestRootPV, best, secondBest );
		
		RootPV = secondBestRootPV;
		best = secondBest;
	}
	else
		dbg_printf("SearchRoot() duchamp no blunder\n" );
	*/
